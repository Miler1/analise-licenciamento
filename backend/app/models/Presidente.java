...
@Entity
@Table(schema="analise", name="presidente")
public class Presidente extends GenericModel {
    ...
    public Presidente(Analise analise, UsuarioAnalise usuario) {
    ...
    public static Presidente distribuicaoAutomaticaPresidente(Analise analise) {
        ...
        String parameter = "ARRAY[" + getParameterLongAsStringDBArray(idsPresidente) + "]";
        ...
        String sql = "WITH t1 AS (SELECT 0 as count, id_usuario, now() as dt_vinculacao FROM unnest(" + parameter + ") as id_usuario ORDER BY id_usuario), " +
                "     t2 AS (SELECT * FROM t1 WHERE t1.id_usuario NOT IN (SELECT id_usuario FROM analise.presidente d) LIMIT 1), " +
                "     t3 AS (SELECT count(id), id_usuario, min(data_vinculacao) as dt_vinculacao FROM analise.presidente " +
                "        WHERE id_usuario in (" + getParameterLongAsStringDBArray(idsPresidente) + ") " +
                "        GROUP BY id_usuario " +
        ...
        return new Presidente(analise, UsuarioAnalise.findById(distribuicaoProcessoVO.id));
