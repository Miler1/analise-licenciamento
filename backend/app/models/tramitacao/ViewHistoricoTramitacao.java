...
public class ViewHistoricoTramitacao extends GenericModel {
	...
	@OneToOne(mappedBy = "viewHistoricoTramitacao")
...
//			joinColumns = @JoinColumn(name = "id_historico_tramitacao"))
	 ...
	 * @return
	 */
	public static List<ViewHistoricoTramitacao> consultarHistoricoTramitacaoView
	(Date dataInicio, Date dataFim, Long acao, Long condicaoFinal){
		List<ViewHistoricoTramitacao> viewHistoricoTramitacao =
				ViewHistoricoTramitacao.find(" dataInicial >=? AND dataInicial <=? AND idCondicaoFinal=? AND idAcao=? ",
						dataInicio,dataFim,condicaoFinal,acao).fetch();
		return viewHistoricoTramitacao;
	}
	 ...
	 * @return
	 */
	public static List<ViewHistoricoTramitacao> consultarHistoricoTramitacaoView(Long acao, Long condicaoFinal){
		List<ViewHistoricoTramitacao> viewHistoricoTramitacao =
				ViewHistoricoTramitacao.find(" idCondicaoFinal=? AND idAcao=? ", condicaoFinal, acao).fetch();
		return viewHistoricoTramitacao;
	}

	public static ViewHistoricoTramitacao getUltimaTramitacao (Long idObjetoTramitavel){

		return ViewHistoricoTramitacao.find("idObjetoTramitavel = :idObjetoTramitavel and idCondicaoInicial != idCondicaoFinal order by dataInicial desc").setParameter("idObjetoTramitavel", idObjetoTramitavel).first();

	}

	public static ViewHistoricoTramitacao getPenultimaTramitacao (Long idObjetoTramitavel){

		List<ViewHistoricoTramitacao> historicosTramitacao = ViewHistoricoTramitacao.find("idObjetoTramitavel = :idObjetoTramitavel and idCondicaoInicial != idCondicaoFinal order by dataInicial desc").setParameter("idObjetoTramitavel", idObjetoTramitavel).fetch();
		return historicosTramitacao.get(1);
	}

	public static List<ViewHistoricoTramitacao> getByObjetoTramitavel (Long idObjetoTramitavel){

		return ViewHistoricoTramitacao.find("idObjetoTramitavel = :idObjetoTramitavel order by dataInicial desc, idHistorico desc")
	...
	public static Date getPrimeiraDataHistoricoTramitacao() {
		...
		String jpql = "SELECT ht.dataInicial "
				+ "FROM " + ViewHistoricoTramitacao.class.getSimpleName() + " ht "
						+ "ORDER BY ht.dataInicial ASC";
		
		Date primeiraData = (Date) ViewHistoricoTramitacao.find(jpql).first();
	...
	public static void setSetor(ViewHistoricoTramitacao viewHistoricoTramitacao, UsuarioAnalise usuarioExecutor) {
		...
		if (usuarioExecutor.usuarioEntradaUnica.setorSelecionado != null) {
			...
			RelHistoricoTramitacaoSetor rel = RelHistoricoTramitacaoSetor.find("historicoTramitacao.id = :x AND siglaSetor = :y")
					.setParameter("x", viewHistoricoTramitacao.idHistorico)
			...
			if (rel == null) {
				...
				rel.viewHistoricoTramitacao = viewHistoricoTramitacao;
