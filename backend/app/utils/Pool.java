...
public class Pool {
    ...
    public Pool(int pool) {
        if (pool > 0)
            ...
            Pool.pool = Integer.parseInt(Play.configuration.getProperty(PROPRIEDADE_POOL, "5"));
        else
            Pool.pool = Runtime.getRuntime().availableProcessors();

        POOL = Executors.newFixedThreadPool(Pool.pool);
    }
